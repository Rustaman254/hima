...
export async function deployWalletOnNetwork(
  walletId: string,
  ...
  switch (chain) {
    ...
    case BlockchainNetwork.CELO: {
      ...
      return await deployWalletOnEVM(walletId, walletAddress, caip2, chainId);
    }
    ...
    case BlockchainNetwork.POLKADOT: {
      ...
      const sender = keyring.addFromUri(process.env.POLKADOT_MNEMONIC!);
